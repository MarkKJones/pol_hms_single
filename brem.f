...
c	real*8 p_ix,p_iy,p_iz
c	real*8 p_fx,p_fy,p_fz,p_fe
	real*8 q2,aprod,adot,ar1,ar2,alpha
	real*8 bei,bef,bee
c	real*8 ami,amf,bpi,bpf,bpp,bepii,bepif,bepfi,bepff
        real*8 dbei,dbef,dbee
c	real*8 dbpi,dbpf,dbpp,dbepii,dbepif,dbepfi,dbepff  
                                          !derivatives of b's
	real*8 b,db
c       real*8 bz,bzz,dbz,dbzz,bsch
	real*8 bsoft,bhard,dbsoft
	real*8 inter,inter_prime
c	logical	radiate_proton

	real*8 k_i_e, k_i_x, k_i_y, k_i_z
	real*8 k_f_e, k_f_x, k_f_y, k_f_z
	real*8 p_i_e, p_i_x, p_i_y, p_i_z
...
! ... compute electron energies
	...
	k_i_e= (k_i_x**2+k_i_y**2+k_i_z**2+ame**2)**0.5
	k_f_e= (k_f_x**2+k_f_y**2+k_f_z**2+ame**2)**0.5

! ........ if he insists on e-m conservation like below, just compute p_i_e
	p_i_e = mp

! ... compute Q2 and masses of initial and final protons

	q2=-1.*( (k_f_e-k_i_e)**2-(k_f_x-k_i_x)**2-
     >		(k_f_y-k_i_y)**2-(k_f_z-k_i_z)**2)
...
! ........ direct initial electron
	...
	aprod= 1.d0
	bei= aprod*(-1./twopi)*log(k_i_e/de)
	dbei= aprod*(-1./twopi)*(-1./de)

! ........ direct final electron

	aprod= 1.d0
	bef= aprod*(-1./twopi)*log(k_f_e/de)
	dbef= aprod*(-1./twopi)*(-1./de)

! ........ e-e interference

	aprod= -1.d0
	adot= k_i_e*k_f_e-k_i_x*k_f_x-k_i_y*k_f_y-k_i_z*k_f_z
	alpha= 2.*ame**2-2.*adot
	ar1= 0.5+sqrt(4.*adot**2-4.*ame**4)/(2.*alpha)
	ar2= 0.5-sqrt(4.*adot**2-4.*ame**4)/(2.*alpha)
	bee= aprod*adot*inter(calculate_spence,alpha,ar1,ar2,k_i_e,k_f_e,de)
...
!-----------------------------------
	...
	del= -2.*alpha/(3.*3.14159)*(-5./3.+x+(1-x/2)*(1+x)**0.5*
