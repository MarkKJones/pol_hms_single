...
cc   Modified from ressf.f by S.Tajima, nov 2006
cc   Change arguments: 
cc   F_one --> Sigma_T, F_L --> Sigma_L when calling resmod.f .
cc   When calculating F_one and F_L, do F_one=sigma_T * ... , F_L=sigma_L * ...
...
cc
cc
cc    SUBROUTINE ressf_2(Q2,W2,F2,R)
      SUBROUTINE ressf_2(W2,Q2, F_one, R, sigma_T, sigma_L) 

      IMPLICIT NONE

      real*8 w2,q2,xval1(50),xvall(50),temp(4)
      real*8 mp,mp2,pi,alpha,xb,F_one,F_L,F2,r,g2
      real*8 sigma_L, sigma_T
      integer i,lu
      logical first/.true./
      integer ntp_events
...
c
      ...
      if ( w2 .ge. (0.9382727+0.136)**2) then
       call resmod(1,w2,q2,xval1,sigma_T)   !!! Transverse Cross Section    !!!

       call resmod(2,w2,q2,xvall,sigma_L)   !!! Longitudinal Cross Section  !!!
      endif
...
CCC  Now convert to structure functions  CCC
      ...
      if ( sigma_T .eq. 0.d0 ) then
        F_one = 0.0
        R=0.d0  
        F2=0.d0
      else
        F_one = sigma_T*(w2-0.9382727*0.9382727)/8./pi/pi/alpha/0.389e3
        F_L = sigma_L*2*xb*(w2-0.9382727*0.9382727)/8./pi/pi/alpha/0.389e3
        R = F_L/ (2.D0 * XB * F_one)
	g2 = (2.d0*xb*mp)**2/q2
